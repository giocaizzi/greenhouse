...
- **The root package `.` MUST set `"component": ""` — do not remove it.** With `package-name: "greenhouse"` and no explicit component, release-please derives the component `greenhouse`, but `include-component-in-tag: false` produces bare `vX.Y.Z` tags whose parsed component is empty. During release discovery release-please matches existing releases to the path **by component**, so `greenhouse` ≠ (empty) → it finds **zero** prior releases (`⚠ Expected 1 releases, only found 0` → `No latest release found … Set(0)`), can't anchor, and aborts (`There are untagged, merged release PRs outstanding`). This — separate from the title pattern — is why v3.0.1 also needed hand-recovery. Forcing `"component": ""` makes the config consistent with the bare tags so discovery finds the latest release and the next release self-tags. (A monorepo like `interviewer` avoids this by using `include-component-in-tag: true` + explicit per-package components, so tags like `api-vX.Y.Z` carry the matching component.)
